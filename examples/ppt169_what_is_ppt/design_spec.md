...
- **Relationships**: membership — 五种含义同归「PPT」一词;link — 右侧概念插图与左侧五行清单逐项对应
...
- **Relationships**: membership — 五个扩展名同属 OOXML 标准族;contrast — 旧版二进制 .ppt 与标准化 .pptx
...
- **Relationships**: order — 转化链五节点按因果先后串联;link — 引线箭头连接各节点
...
- **Relationships**: membership — 九项沟通任务平行并列;order — 结论带指出主次与顺序
...
- **Relationships**: membership — 七项属性叠加成 PowerPoint 的独特价值
...
- **Relationships**: contrast — 左列首要需求与右列更合适的媒介逐行对照;末行 PowerPoint 场景反衬前六行
...
- **Relationships**: order — 四阶段按时间先后排列;link — 同一份 deck 贯穿四栏
...
- **Relationships**: membership — 四种传递场景并列;contrast — 演讲者主导与读者主导两种设计取向
...
- **Relationships**: parent — Theme → Master → Layout → Slide 的层级包含;link — 标注引线把插图部件与模块代号连起来
...
- **Relationships**: membership — 八个部件各归其职;contrast — 「视觉正确」与「结构良好」
...
- **Relationships**: contrast — Theme 与 Template 对比条;parent — 四层自下而上的堆叠包含
...
- **Relationships**: order — 六层质量自下而上分层;link — 左侧清单与右侧剖面插图逐层对应
...
- **Relationships**: membership — 三项研究并列印证同一结论;contrast — 与「图片越多越好」的直觉相反
...
- **Relationships**: contrast — 八条「不是什么」反衬 PPT 应是什么;membership — 八条否定同归一类误解
...
- **Relationships**: membership — 八个问题平行并列;order — 结论带说明它们先于风格决定
...
- **Relationships**: order — 方法闭环四步首尾相接;parent — 中心核心句统领闭环带
