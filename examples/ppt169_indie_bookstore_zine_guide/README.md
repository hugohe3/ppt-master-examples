...
- Runtime: Default Generate (Strategist → Image_Generator → Executor) from a zine-culture guide compiled from twenty public sources listed on the credits page
- Visual style: Risograph zine (mis-registered overprint titles, grain patterns, limited ink palette); this deck predates the mode / visual-style catalogs, so `spec_lock.md` carries no `mode` or `visual_style` section
- Image system: 10 AI illustrations generated in-pipeline, 18 SVG pattern fills for the riso grain
- Native objects: 14 chunk-filled icons; 87 entrance animations (appear / box / fade / push / wipe)
- Speaker notes on every slide (18 pages)
...
python3 skills/ppt-master/scripts/svg_to_pptx.py <this_dir> -o <this_dir>/exports/indie_bookstore_zine_guide.pptx
```
