...
- Runtime: Default Generate (Strategist → Image_Generator → Executor) from a tech-talk outline on taking AI agents from demo to production
- Visual style: Glassmorphism SaaS tech talk (frosted panels, aurora backgrounds, depth layering); this deck predates the mode / visual-style catalogs, so `spec_lock.md` carries no `mode` or `visual_style` section
- Image system: 8 AI background images generated in-pipeline
- Native objects: 28 phosphor-duotone icons, 74 gradient definitions building the glass panels; 103 entrance animations (expand / fade / wipe / zoom)
- Speaker notes on every slide (12 pages)
...
- `06_tool_orchestration.svg` / `11_roadmap.svg`: two gradient-stroked `<line>` elements (which cannot export) became a solid dashed line and a gradient-filled 4 px rect
## Directories
...
python3 skills/ppt-master/scripts/svg_to_pptx.py <this_dir> -o <this_dir>/exports/glassmorphism_demo.pptx
```
