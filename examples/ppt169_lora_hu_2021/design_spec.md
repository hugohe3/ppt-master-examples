...
| -------- | --------- | ----- | ------- | ---- | -------------- | ----------- | ------ | --------- | ----------- | --------- |
| cover_hero.jpg | 1280x720 | 1.78 | 封面主视觉(Slide 01) | Illustration | #1 full-bleed background with floating title + #29 two-stop scrim | ai | Pending | 冻结的大型预训练权重方阵被锁住,旁路注入一条由两个细长低秩矩阵(降维 A、升维 B)组成的可训练分支,连线汇合到输出;蓝图示意,留下方安静带给 SVG 标题 | embedded | hero_page |
| cost_explosion.jpg | 860x720 | 1.19 | 全量微调部署代价(Slide 03) | Diagram | #4 right image bleeding off the canvas edge + #32 multi-stop scrim with hue shift | ai | Pending | 同一个 175B 巨型模型被为每个下游任务整份复制成多座沉重的服务器机柜,强调"每任务一份满参数副本"的高昂存储/部署成本 | embedded | local |
| lowrank_insight.jpg | 1280x720 | 1.78 | 低秩内在维度洞察(Slide 05) | Illustration | #19 image floating in whitespace with thin frame and caption + #21 rounded rectangle crop | ai | Pending | 高维权重更新云坍缩贴合到一张低维子空间薄片上,少数主方向承载主要信息;概念示意,大量留白 | embedded | local |
| reparam_diagram_cropped.jpg | 1280x531 | 2.41 | LoRA 重参数化结构图(Slide 06) | Diagram | #44 background image + native network/architecture diagram + #41 measurement lines and module tags (engineering overlay) | ai | Pending | 输入 x 同时进入冻结的预训练权重 W 与并行低秩旁路:先经降维矩阵 A 到秩 r 瓶颈,再经升维矩阵 B,两路输出相加得 h;箭头清晰,瓶颈维度 r 标注 | embedded | local |
| attention_apply.jpg | 530x580 | 0.91 | 作用于 Transformer 注意力(Slide 08) | Diagram | #2 left-third image + right text body + #21 rounded rectangle crop | ai | Pending | 一个 Transformer 自注意力模块,四个投影矩阵 Wq/Wk/Wv/Wo,其中 Wq 与 Wv 各挂一条低秩 LoRA 旁路高亮,MLP 模块标为冻结;蓝图结构 | embedded | local |
| subspace_heatmap.jpg | 1024x1024 | 1.0 | 子空间相似度热力图(Slide 14) | Diagram | #3 right-third image + left text body + #21 rounded rectangle crop | ai | Pending | 方阵热力图:行 i 列 j 的归一化子空间相似度,左上角(top singular directions)高强度发亮、其余迅速变暗,清晰的网格单元,蓝-橙强度色阶 | embedded | local |
...
#### Slide 01 - Cover
- **Audience move**: 不知道这场讲解讲什么 → 知道对象是 Hu et al. 2021 的 LoRA 论文,以及「低秩适配」四个字指向什么
- **Relationships**: none
...
#### Slide 02 - 演示路线
- **Audience move**: 知道题目 → 掌握六段路线:问题、局限、洞察、方法、结果、理解
- **Relationships**: order — 六个议程项按讲解顺序排列
...
#### Slide 03 - 问题:全量微调的代价
- **Audience move**: 以为微调只是算力问题 → 明白 175B 尺度下每个任务一份参数副本的存储与切换代价才是不可行的根源
- **Relationships**: contrast — 单个模型体量与「每任务一份副本」的倍增关系
...
#### Slide 04 - 已有方法的局限
- **Audience move**: 以为已有高效方法够用 → 看到 Adapter、Prefix 各自的代价(延迟/序列长度)和普遍的质量落差
- **Relationships**: membership — 三类已有方法各自归入「额外延迟 / 占用输入 / 质量不及全量」三种代价
...
#### Slide 05 - 核心洞察:低"内在秩"
- **Audience move**: 接受问题 → 接受核心假设:权重更新 ΔW 本身内在低秩,所以可以用极低的 r 表达
- **Relationships**: link — Aghajanyan 2020 的已有证据引出本文的低秩假设
...
#### Slide 06 - LoRA 方法
- **Audience move**: 知道假设 → 看懂 LoRA 的重参数化:冻结 W0,只训练 A、B 两个低秩矩阵
- **Relationships**: parent — W0 冻结主干与 BA 旁路的父子关系;link — 公式 h = W0x + BAx 连接两支
...
- **Audience move**: 看懂结构 → 掌握三个实现细节:α/r 缩放、部署时合并、任务切换只换 BA
- **Relationships**: membership — 三个实现细节归为同一方法的工程约定
- **Layout**: 公式 + 要点(dense)
- **Title**: 一个常数 α、一次部署合并
...
- **Audience move**: 知道怎么实现 → 知道作用在 Transformer 的哪里:注意力四个投影里取 Wq、Wv,以及由此的参数量公式
- **Relationships**: parent — Transformer 注意力块包含四个投影;contrast — 适配与不适配的权重
...
#### Slide 09 - 四大优势
- **Audience move**: 理解方法 → 能复述四个优势:可共享、高效训练、零推理延迟、可与 prefix 叠加
- **Relationships**: membership — 四个优势并列归属同一方法
...
#### Slide 10 - 推理零延迟
- **Audience move**: 听说零延迟 → 看到 Adapter 在不同 batch/序列长度下增加的延迟百分比,LoRA 为零
- **Relationships**: contrast — Adapter 两种变体与 LoRA/FT 的延迟差
...
#### Slide 11 - 实验设置
- **Audience move**: 准备看结果 → 知道实验覆盖 RoBERTa、DeBERTa、GPT-2、GPT-3 四类模型与对应任务
- **Relationships**: membership — 四类模型归入 NLU/NLG 两组评估
...
#### Slide 12 - GLUE 结果
- **Audience move**: 等结果 → 看到 GLUE 上 LoRA 以 1/300~1/400 的可训练参数持平或更优
- **Relationships**: contrast — FT 与 LoRA 在同一模型上的参数量与得分对比
...
- **Audience move**: 信服中小模型结果 → 看到 175B 尺度上 10,000× 参数缩减、350GB→35MB、+25% 吞吐仍成立
- **Relationships**: contrast — 全量微调与 LoRA 在四项指标上的量级差
...
#### Slide 14 - 低秩理解:权重与秩
- **Audience move**: 知道有效 → 理解为什么有效:适配 Wq+Wv 优于单个,r=1 已够,子空间高度重叠
- **Relationships**: order — r 从 1 到 64 的得分序列;contrast — 不同权重组合的得分
...
#### Slide 15 - 结论与影响
- **Audience move**: 看完证据 → 带走三点:方法本身、工程意义、行业影响,以及论文与代码入口
- **Relationships**: membership — 三条结论归属同一收束;link — 指向论文与仓库
